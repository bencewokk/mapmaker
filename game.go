...
func offsetsx(tobeoffset float32) float32 {
	return ((tobeoffset + globalGameState.camera.pos.float_x) + screenWidth/2)
}
func offsetsy(tobeoffset float32) float32 {
	return ((tobeoffset + globalGameState.camera.pos.float_y) + screenHeight/2)

}

func coffsetsx(tobeoffset float32) float32 {
	return (tobeoffset - globalGameState.camera.pos.float_x - screenWidth/2)
}
func coffsetsy(tobeoffset float32) float32 {
	return (tobeoffset - globalGameState.camera.pos.float_y - screenHeight/2)
